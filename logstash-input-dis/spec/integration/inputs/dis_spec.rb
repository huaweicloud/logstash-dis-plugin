...
describe "inputs/dis", :integration => true do
  ...
  describe "#kafka-topics" do
    ...
    it "should consume all messages from plain 3-partition topic" do
      dis_input = LogStash::Inputs::Dis.new(plain_config)
      queue = Queue.new
      t = thread_it(dis_input, queue)
      begin
        t.run
    ...
    it "should consume all messages from snappy 3-partition topic" do
      dis_input = LogStash::Inputs::Dis.new(snappy_config)
      queue = Queue.new
      t = thread_it(dis_input, queue)
      begin
        t.run
    ...
    it "should consume all messages from lz4 3-partition topic" do
      dis_input = LogStash::Inputs::Dis.new(lz4_config)
      queue = Queue.new
      t = thread_it(dis_input, queue)
      begin
        t.run
    ...
    it "should consumer all messages with multiple consumers" do
      dis_input = LogStash::Inputs::Dis.new(multi_consumer_config)
      queue = Queue.new
      t = thread_it(dis_input, queue)
      begin
        t.run
        wait(timeout_seconds).for {queue.length}.to eq(num_events)
        expect(queue.length).to eq(num_events)
        dis_input.kafka_consumers.each_with_index do |consumer, i|
  ...
  describe "#kafka-topics-pattern" do
    ...
    it "should consume all messages from all 3 topics" do
      dis_input = LogStash::Inputs::Dis.new(pattern_config)
      queue = Queue.new
      t = thread_it(dis_input, queue)
      begin
        t.run
  ...
  describe "#kafka-decorate" do
    ...
    it "should show the right topic and group name in decorated kafka section" do
      ...
      dis_input = LogStash::Inputs::Dis.new(decorate_config)
      queue = Queue.new
      t = thread_it(dis_input, queue)
      begin
        t.run
  ...
  describe "#kafka-offset-commit" do
    ...
    it "should manually commit offsets" do
      dis_input = LogStash::Inputs::Dis.new(manual_commit_config)
      queue = Queue.new
      t = thread_it(dis_input, queue)
      begin
        t.run
